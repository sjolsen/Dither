...
func _run_pretreat(cpass: _Pass) -> void:
	...
	var size: Vector2i = cpass.buffers.get_internal_size()
	...
	var wg_x := (size.x + 31) / 32
	var wg_y := (size.y + 31) / 32
	...
	_rd.compute_list_set_push_constant(cpass.compute_list, pc, pc.size())
